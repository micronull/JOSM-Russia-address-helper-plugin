...
## Добавлено
### 0.9.7.0
- функционал массовой загрузки геометрии по выделенной линии
- добавлен функционал авто-ортогонализации (оквадрачивания) импортированной геометрии и его настройки
- если линия выделения содержит больше точек, чем задан лимит запросов, линия будет автоматически разрезана, а остаток - выделен после завершения операции
- добавлена опция создания адресной точки, если при запросе по линии не найдена геометрия здания 
- добавлена кнопка "Добавить из значений по умолчанию" в настройки стоп-слов валидации адреса
### 0.9.6.7
- разделены настройки предела фильтрации импортируемой геометрии по площади для зданий и границ
### 0.9.6.6
- к подключаемым слоям НСПД добавлен слой административных границ НП и слой границ муниципальных образований
- геометрию со слоя административных границ можно импортировать с помощью инструмента "Запрос по клику" с нажатой клавишей ALT при клике
- добавлен новый тип НП для сопоставления - железнодорожная станция
- кнопки действий по загрузке данных неактивны, если нет слоя с данными ОСМ
### 0.9.6.4
- импорт геометрии по клику со слоя недостроенных объектов.
- длительность таймаута запроса к АПИ снижена до 3 секунд и вынесена в настройки 
### 0.9.5.1
...
## Изменено
### 0.9.7.0
- удалена (всегда включена) настройка "Записывать сырой адрес из ЕГРН в тег addr:RU:egrn"
- удалена (всегда включена) настройка "Импортировать геометрию из ПКК"
- в стоп-лист распознавания адресов добавлены "за границами", "позиция", "поз."
### 0.9.6.8
- если здание уже импортировано со слоя зданий (режим по клику), не импортируем его со слоя незаконченных зданий и/или сооружений
### 0.9.6.7
- исправлен вылет, если не удалось импортировать полученную геометрию или в результате фильтрации нет ни одного полигона
- исправлена опечатка в README, thx @Бобровоз
### 0.9.6.6
- исправлен вылет, когда в получаемых данных приходят поля intersectedCadNumbers или unitedCadNumbers
- изменения в документации - исправлен конфиг nginx для случая, если в системе работает IPv6 (спасибо @alpetukhov)
- переработан механизм "зачистки" данных при загрузке, теперь он должен корректно удалять мультиполигоны, линии и точки, если на них есть тэг fixme=REMOVE ME!
### 0.9.6.4
- теперь к импортированным домам по клику дописывается "source:addr=ЕГРН"
- количество попыток при таймауте/сбое запроса снижено до 2х (более ранний выход из процедуры запроса если АПИ лежит или забанило)
### 0.9.6.2
- обход зависания, если массовый запрос производится на отфильтрованном слое, и плагин пытается выделить отфильтрованные здания. Теперь выделение просто сбрасывается после массового запроса.
### 0.9.4.8
...
## Исправлено
### 0.9.7.1
- исправлен вылет при импорте мультиполигона, не допускающего ортогонализации ни одной своей части
- исправлено зависание при попытке плагина выделить объекты, отфильтрованные в редакторе. Убрана связанная скрытая настройка
- исправлено разбиение длинных строк значений разрешенного использования
### 0.9.7.0
- длинные значения (более 255 символов) полей ЕГРН разбиваются на несколько тэгов, помеченных через постфикс ":p1", ":p2" и т.д.
- фильтрация дублей точек в импортированной геометрии
- ошибка в валидаторе конфликта данных. Теперь новые и старые значения тэгов не должны быть перепутаны.
### 0.9.6.4
- импортированная геометрия теперь попадает в валидатор "Адрес добавлен"
- импортированной геометрии дописывается корректный "addr:RU:egrn"
### 0.9.6.3
- вылет при попытке собрать из импортированной геометрии полигон, если геометрия кривая (пересечение полигонов)
### 0.9.6.2
- вылет при удалении последнего слоя в редакторе (привет, addLayerChangeListener, я смотрю на тебя)
### 0.9.6.1
...
- обработка ситуации "нумерованная улица/место распознана как номер дома"