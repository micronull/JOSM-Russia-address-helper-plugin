...
При успешном разборе и сопоставлении на каждый найденный адрес в месте клика создается по точке с тэгами (`addr:housenumber`, `addr:street` или `addr:place`).
...
С версии 0.9.7.0 импортированная геометрия подвергается ортогонализации ("оквадрачиванию") в соответствии с настройками.
Алгоритм ортогонализации применяется к контуру здания, если все его углы отклоняются от 90 или 180 градусов не более чем на заданное в настройках значение.
Если результат оквадрачивания вас не устраивает, его можно откатить через Undo (`Ctrl+Z`).
...
С версии 0.9.7.0 появилась возможность массовой загрузки геометрии зданий из ЕГРН вместе с атрибутивной информацией. Этот режим является своего рода комбинацией режимов "Запрос по клику" и "Массовая загрузка".
Для массовой загрузки геометрии необходимо, при подключенном слое Зданий ЕГРН, нарисовать линию, точки которой находятся в контуре зданий, которые нужно загрузить. Проще говоря, в режиме рисования линий, "прокликать" все нужные дома. Затем, выделить эту линию и нажать кнопку "Массовая загрузка".
Здания будут загружены, и, по возможности, им будет присвоена адресная информация, а так же дополнительные тэги ЕГРН. Линия, использованная для загрузки, автоматически удалится после запроса данных.
Если при этом количество точек линии превышает лимит запроса, установленный в ЕГРН, плагин автоматически разрежет линию на две части, первая из которых меньше лимита, а вторая не удалится после запроса.
При включенной опции "Создавать адресные точки если геометрия не найдена", в тех местах, где не удалось загрузить контур здания, но есть адресная информация, будет создана точка с этими данными.
Импортированная геометрия подвергается ортогонализации ("оквадрачиванию") в соответствии с настройками.
...
5. Выберите в JOSM пункт меню `Данные - Загрузка адресов из ЕГРН - Добавить слои WMS ПКК`. В настройках слоев добавятся 6 новых слоев, с данными ЕГРН.
6. Добавьте нужные вам слои в отображаемые через меню `Слои`.
Примечание: если на порту, прописанном в конфигурации (8081) уже работает какая-то программа (порт занят), nginx не запустится. Справедливо и обратное - после запуска nginx, могут отказаться запускаться другие программы, пытающиеся использовать тот же порт.
...
4. Плагин обработает полученные данные по следующему алгоритму:
    1. Если у здания/участка есть адрес, и он успешно разобран на составляющие, и для распознанных объектов нашлось соответствие в данных ОСМ (найдена улица, либо точка населенного пункта/квартала/микрорайона), и не найден дубликат, то зданию будет присвоен адрес.
...
Еще одним валидным источником границ НП является система [ФГИС ТП](https://fgistp.economy.gov.ru/), где аккумулируются результаты работы местных властей по планированию территории. Ими можно пользоваться для нанесения или уточнения границы, обращая, впрочем, внимание на дату документа (генплан НП или поселения может быть сильно устаревшим, а новый еще не выложен и/или не разработан).
...
4. Те же оговорки относятся к расширенным данным ЕГРН - этажности, типе здания и дате начала эксплуатации. Например, для исторических церквей может быть как валидная дата постройки, так и дата реконструкции и/или начала работы храма уже в современное время. Этажность на примере храмов практически всегда = 1, что формально верно, но не соответствует модели высот ОСМ.
> [!IMPORTANT]
> Вывод - приоритет во внесении данных ОСМ всегда имеет truth on the ground - проверенная на местности адресная табличка или данные, или хотя бы увиденная на снимках разрешенных к использованию панорам (Яндекс). Никакие другие картографические материалы, за исключением может быть официальных документов сельских, городских администраций,
...
При запросе геометрии (по клику с нажатым ALT) возвращаются следующие поля:

| Тэг         | Описание                                                                  |
|-------------|---------------------------------------------------------------------------|
| description | Текстовое описание геометрии, для границ НП будет указано наименование НП |
| docName     | Наименование документа установившего геометрию границы                    |
| date        | Дата документа                                                            |
...
- Оквадрачивать импортированную геометрию (автоматический вызов операции редактора по нажатию `Q`). Настройка применяется к запросу по клику и массовому запросу по линии. Если все углы импортированного здания отличаются от 90/180 градусов не больше чем на N градусов, к зданию применяется команда ортогонализации. Эта команда применяется последней и может быть отменена по Undo.
- "Оквадрачивать, если углы отличаются..." - настройка алгоритма ортогонализации. Значение в 10 градусов показывает хороший результат.
...
## Настройки запроса по клику
 ...
 - Объединять данные слоев на одной точке - атрибутивные данные здания и участка будут объединены на одной точке, при этом дублирующимся тэгам будет приписан суффикс слоя, откуда получены данные и номер объекта, если их было получено больше одного.
...
## Настройки валидации
- таблица стоп-слов. Если адрес ЕГРН включает подстроку из этой таблицы, адрес не будет распознаваться, и будет сгенерирована ошибка валидации. Кнопка "Обновить данные" **добавляет** значения из списка значений по умолчанию, хранящегося внутри плагина.
