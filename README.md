...
С версии 0.9.6.6 запрос по клику позволяет импортировать административные границы населенных пунктов.
...
6. Процесс работы
   ...
   3. добавление административных границ населенных пунктов:
   - подключите слой *Административные границы НП*. На карте отобразятся синей пунктирной линией границы населенных пунктов.
   - уменьшите масштаб карты чтобы интересующий вас НП целиком был виден в окне редактора
   - выберите инструмент плагина "Запрос по клику" и, удерживая клавишу `ALT`, кликните в любом месте внутри границы НП.
   - плагин импортирует геометрическую информацию о границе в виде полигона или мультиполигона, учитывая сдвиг слоя, выбранного за основу в настройках. При этом на полигоне/мультиполигоне будут присутствовать тэги "fixme=REMOVE ME!", данные об источнике геометрии "source:geometry=ЕГРН" и дате документа ("source:geometry:date"), устанавливающего геометрию границы.
   - если вы хотите использовать импортированную границу, тэг "fixme=REMOVE ME!" необходимо удалить! Иначе граница будет удалена плагином перед загрузкой на сервер. Тэги, начинающиеся с префикса "autoremove:" удалять не надо, они будут удалены автоматически.
   - если граница уже существует в ОСМ, то можно перенести тэги с нее на импортированную, и существующую удалить. Так же в модулях JOSM есть очень удобный инструмент "Замена геометрии", который может преобразовать старую границу по геометрии новой. Однако, это стоит делать если старая граница явно некорректная или слишком грубая.
   - если границы не существует в ОСМ, то нужно скопировать основные тэги НП с точки НП на границу НП (place=*, name=*, остальные по желанию)
   - для смежных НП границу желательно преобразовать в мультиполигон и обьединить общую часть
   ...
   Часто граница ЕГРН импортируется мультиполигоном, из которого вырезаны мелкие дырки, например опоры электросетей или другая инфраструктура. Я предпочитаю конвертировать такие мультиполигоны в полигоны, оставляя только внешний контур.
