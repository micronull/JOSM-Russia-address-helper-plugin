...
internal class StreetNameParserTest {
    ...
    fun parseTest() {
        ...
        testData.forEach { osmStreet, egrnStreet ->
            assertEquals(
                osmStreet,
                ParsedStreet.identify(egrnStreet, streetTypes, testData.keys.map { Pair(it, it) }.toMap()).name
            )
        }

        assertEquals("улица Пупкина",
            ParsedStreet.identify("Москва сити, улица Васимира Пупкина", streetTypes, mapOf("улица Васимира Пупкина" to "улица Пупкина")).name)
    }
}